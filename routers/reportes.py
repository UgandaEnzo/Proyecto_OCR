...
def exportar_reportes(tipo_reporte: str='mensual', format: str='xlsx', start_date: Optional[datetime]=None, end_date: Optional[datetime]=None, db: Session=Depends(get_db)):
    ...
    if formato == 'xlsx':
        contenido = _crear_excel_reporte(resultados, pagos_detalle, tipo_reporte, start_date, end_date, empresa_nombre, color_primario, color_secundario, logo_bytes)
        media_type = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    else:
        contenido = _crear_pdf_reporte(resultados, pagos_detalle, tipo_reporte, start_date, end_date, empresa_nombre, color_primario, color_secundario, logo_bytes)
